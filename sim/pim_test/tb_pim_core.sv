...
module tb_pim_core;
    ...
    localparam int  DONE_WORD       = 7;             // sram_ctrl.mem_array index
    localparam int  RESULT_ADD_WORD = 5;             // add.p result word index
    localparam int  RESULT_MUL_WORD = 6;             // mul.p result word index
    localparam [31:0] DONE_SENTINEL  = 32'h0000_DEAD;
    localparam [31:0] EXPECTED_ADD   = 32'd10;
    localparam [31:0] EXPECTED_MUL   = 32'd8;
    ...
    initial begin
        ...
        forever begin
            ...
            if (u_pim_subsystem.u_sram_ctrl.mem_array[DONE_WORD] === DONE_SENTINEL) begin
                $display("[%0t ns] Done sentinel detected at cycle %0d.",
                         $time, cycle_count);

                // Read results from SRAM
                result_add = u_pim_subsystem.u_sram_ctrl.mem_array[RESULT_ADD_WORD];
                result_mul = u_pim_subsystem.u_sram_ctrl.mem_array[RESULT_MUL_WORD];

                $display("[%0t ns] --------------------------------------------------------", $time);
                $display("[%0t ns]  add.p : actual = %0d  expected = %0d  => %s",
                         $time, result_add, EXPECTED_ADD,
                         (result_add === EXPECTED_ADD) ? "PASS" : "FAIL");
                $display("[%0t ns]  mul.p : actual = %0d  expected = %0d  => %s",
                         $time, result_mul, EXPECTED_MUL,
                         (result_mul === EXPECTED_MUL) ? "PASS" : "FAIL");
                $display("[%0t ns] --------------------------------------------------------", $time);

                if ((result_add === EXPECTED_ADD) && (result_mul === EXPECTED_MUL))
                    $display("[%0t ns]  OVERALL: PASS", $time);
                else
                    $display("[%0t ns]  OVERALL: FAIL", $time);
